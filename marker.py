...
def getArgs():
    ...
    parser.add_argument("-i", "--image_path", type=str, required=True, help="Path for the image to be watermarked.")
    parser.add_argument("-w", "--watermark_path", type=str, required=True, help="Path for the watermark.")
    parser.add_argument(
        "-o",
        "--result_path",
        type=str,
        required=False,
        default="results",
        help="Directory where watermarked image needs to be stored.",
    )
    parser.add_argument(
        "-p",
        "--plane",
        choices=[i + 1 for i in range(8)],
        type=int,
        required=False,
        default=1,
        help="Plane at which encoding is to be done.",
    )
    parser.add_argument(
        "-b",
        "--block_size",
        ...
        required=False,
        default=3,
        help="Block size with which encoding is to be done.",
    )
